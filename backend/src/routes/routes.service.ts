...
export class RoutesService {
      ...
      const createdRoute = await tx.query.routes.findFirst({
        ...
        with: {
          stops: {
            ...
            orderBy: (routeStops: any, { asc }: any) => [
  ...
  async update(id: string, updateRouteDto: UpdateRouteDto) {
    ...
    if (updateRouteDto.vehicleId)
      updateData.vehicle_id = updateRouteDto.vehicleId;
    ...
    if (updateRouteDto.destination)
      updateData.destination = updateRouteDto.destination;
    if (updateRouteDto.estimated_duration)
      updateData.estimated_duration_mins = updateRouteDto.estimated_duration;

    const [updatedRoute] = await this.db
      .update(routes)
      .set(updateData)
      .where(eq(routes.id, id))
      .returning();
