...
export const tripBoardingLogs = pgTable('trip_boarding_logs', {
  ...
  stop_id: uuid('stop_id')
    .references(() => stops.id)
    .notNull(),
  status: boardingStatusEnum('status')
...
export const tripGPSLogs = pgTable(
  'trip_gps_logs',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    trip_id: uuid('trip_id')
  ...
  (table) => {
    return {
      tripTimestampIdx: index('trip_gps_log_trip_timestamp_idx').on(
        table.trip_id,
        table.timestamp,
      ),
    };
...
export const tripBoardingLogsRelations = relations(
  tripBoardingLogs,
  ({ one }) => ({
    trip: one(trips, {
