...
export class TripGeneratorService {
  ...
  async generateDailyTrips(targetDate?: Date) {
      ...
      if (!existingTrip) {
        ...
        if (assignment.employees && assignment.employees.length > 0) {
          ...
          if (routeWithStops && routeWithStops.stops.length > 0) {
            ...
            const boardingLogs = assignment.employees
              .map((emp) => ({
                trip_id: newTrip.id,
                employee_id: emp.employee?.id, // Use the ID from employees table directly
                stop_id: firstStopId,
            ...
            if (boardingLogs.length > 0) {
              await this.db
                .insert(schema.tripBoardingLogs)
                .values(boardingLogs);
            }
          }
        }
    ...
    this.logger.log(
      `Generated ${createdTrips.length} new trips for ${dateStr}`,
    );
