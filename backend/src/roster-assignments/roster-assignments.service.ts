...
export class RosterAssignmentsService {
  ...
  async create(createAssignmentDto: any) {
    const { driverId, vehicleId, routeId, date, shiftType } =
      createAssignmentDto;
    ...
    // Assign employees
    if (
      createAssignmentDto.employeeIds &&
      createAssignmentDto.employeeIds.length > 0
    ) {
  ...
  async update(id: string, updateAssignmentDto: any) {
    ...
    if (updateAssignmentDto.tripType)
      updateData.trip_type = updateAssignmentDto.tripType;
    if (updateAssignmentDto.scheduledTime)
      updateData.scheduled_time = updateAssignmentDto.scheduledTime;
    ...
    if (updateAssignmentDto.employeeIds !== undefined) {
      await this.db
        .delete(rosterAssignmentEmployees)
  ...
  private async updateRotationTracking(
    ...
    if (!tracking) {
      ...
      await this.db
        .update(rotationTracking)
        .set({
          weekend_count: tracking.weekend_count + (isWeekend ? 1 : 0),
          night_shift_count:
            tracking.night_shift_count + (isNightShift ? 1 : 0),
