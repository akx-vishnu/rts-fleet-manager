...
export class FleetService {
  ...
  async updateDriver(id: string, updateDriverDto: any) {
    ...
    try {
      ...
      if (updateDriverDto.license_number)
        driverUpdateData.license_number = updateDriverDto.license_number;
      if (updateDriverDto.status)
        driverUpdateData.status = updateDriverDto.status;
    ...
    try {
      ...
      if (error.code === '23503' || error.cause?.code === '23503') {
        throw new ConflictException(
          'Cannot delete driver with existing trips or assignments',
        );
      }
      throw error;
    }
  ...
  async updateVehicle(id: string, updateVehicleDto: any) {
    ...
    if (updateVehicleDto.license_plate)
      updateData.license_plate = updateVehicleDto.license_plate;
    if (updateVehicleDto.model) updateData.model = updateVehicleDto.model;
    if (updateVehicleDto.capacity)
      updateData.capacity = updateVehicleDto.capacity;
  ...
  async removeVehicle(id: string) {
    ...
    try {
      ...
      if (error.code === '23503' || error.cause?.code === '23503') {
        throw new ConflictException(
          'Cannot delete vehicle with existing assignments or trips',
        );
      }
      throw error;
    }
    ...
    await this.db
      ...
      .set({
        ...
        distance_traveled_km:
          distance && distance > 0
            ? distance
            : currentTrip?.distance_traveled_km || 0,
        updated_at: new Date(),
      })
      .where(eq(schema.trips.id, tripId));
  ...
  async updateEmployeeStatus(
    ...
    if (existingLog) {
      // Update
      await this.db
        .update(schema.tripBoardingLogs)
        .set({
          status: status,
